...
Choose a calibrator based on the input scale, the amount of calibration
data, and the shape of the expected miscalibration. Binary methods take
vectors, and multiclass methods take matrices with one column per class.

| Method | Input scale | Useful when | Main caution |
|----|----|----|----|
| [`cal_platt()`](https://prdm0.github.io/probcal/reference/cal_platt.md) | score or probability | the calibration curve is close to logistic | limited flexibility |
| [`cal_temperature()`](https://prdm0.github.io/probcal/reference/cal_temperature.md) | logits | a model is overconfident but ranking is useful | requires logits, not probabilities |
| [`cal_beta()`](https://prdm0.github.io/probcal/reference/cal_beta.md) | probability | probabilities have asymmetric distortion | clips `0` and `1` before fitting |
| [`cal_isotonic()`](https://prdm0.github.io/probcal/reference/cal_isotonic.md) | probability | many calibration observations are available | can overfit small calibration sets |
| [`cal_histogram()`](https://prdm0.github.io/probcal/reference/cal_histogram.md) | probability | interpretability by bins is preferred | depends on bin choice |
...
expects logits because it estimates a scalar temperature in logit space.
...
The example below simulates probabilities that are too confident. The
raw probabilities and calibrated probabilities are compared on the same
test set.
...
Start with
[`cal_beta()`](https://prdm0.github.io/probcal/reference/cal_beta.md) or
[`cal_platt()`](https://prdm0.github.io/probcal/reference/cal_platt.md)
for small to moderate calibration sets. Use
[`cal_temperature()`](https://prdm0.github.io/probcal/reference/cal_temperature.md)
...
when the calibration set is large enough to support a flexible monotone
curve. Use
[`cal_histogram()`](https://prdm0.github.io/probcal/reference/cal_histogram.md)
when a bin-level rule is easier to audit or explain.

Cross-validated calibration is useful when a separate calibration set
would be too small.
